...
public class JacksonTest {
    ...
    public void ifSignMustHaveKeys() throws URISyntaxException, UnknownHostException, InterruptedException {
        ...
        GossipManager gossipService = GossipManagerBuilder.newBuilder().cluster(cluster).uri(uri).id(
                1 + "").gossipMembers(startupMembers).gossipSettings(settings).build();
        gossipService.init();
    }
    ...
    public void jacksonSerialTest() throws InterruptedException, URISyntaxException, IOException {
        ...
        @SuppressWarnings("unchecked") OrSet<Integer> back = objectMapper.readValue(s, OrSet.class);
    ...
    void jacksonCrdtSeDeTest(Object value, Class<?> cl) {
        ...
        try {
            ...
            @SuppressWarnings("unchecked") Object parsedValue = objectMapper.readValue(valueS, cl);
    ...
    public void testMessageSerializationRoundTrip() throws Exception {
        ProtocolManager mgr = new JacksonProtocolManager(
