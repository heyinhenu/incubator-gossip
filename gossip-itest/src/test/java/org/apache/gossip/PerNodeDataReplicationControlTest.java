...
public class PerNodeDataReplicationControlTest extends AbstractIntegrationBase {
    ...
    @Test
    public void perNodeDataReplicationTest() throws InterruptedException, UnknownHostException, URISyntaxException {
        ...
        nodes.get(0).gossipPerNodeData(getPerNodeData("public", "I am visible to all", new AllReplicable<>()));
        // Adding new per node data to Node 1 with no replication (replicate none)
        nodes.get(0).gossipPerNodeData(getPerNodeData("private", "I am private", new NotReplicable<>()));
        ...
        nodes.get(0).gossipPerNodeData(getPerNodeData("wl", "white list", new WhiteListReplicable<>(whiteList)));
    ...
    @Test
    public void perNodeDataDcReplicationTest() throws InterruptedException, UnknownHostException, URISyntaxException {
        ...
        createDcNode(URI.create("udp://127.0.0.1:5001"), "1", settings, seeder, cluster, "DataCenter1", "Rack1");
        createDcNode(URI.create("udp://127.0.0.1:5002"), "2", settings, seeder, cluster, "DataCenter1", "Rack2");

        createDcNode(URI.create("udp://127.0.0.1:5006"), "6", settings, seeder, cluster, "DataCenter2", "Rack1");
        createDcNode(URI.create("udp://127.0.0.1:5007"), "7", settings, seeder, cluster, "DataCenter2", "Rack1");
        ...
        nodes.get(0).gossipPerNodeData(getPerNodeData("Dc1Rack1", "I am belong to Dc1", new DataCenterReplicable<>()));
        // Node 6 has a shared key with 'Dc2Rack1'
        nodes.get(2).gossipPerNodeData(getPerNodeData("Dc2Rack1", "I am belong to Dc2", new DataCenterReplicable<>()));
    ...
    private void createDcNode(
            URI uri, String id, GossipSettings settings, RemoteMember seeder, String cluster, String dataCenter,
            String rack
    ) {
        ...
        GossipManager dcNode = GossipManagerBuilder.newBuilder().cluster(cluster).uri(uri).id(id).gossipSettings(
                settings).gossipMembers(Arrays.asList(seeder)).properties(props).build();
        dcNode.init();
        register(dcNode);
    }
