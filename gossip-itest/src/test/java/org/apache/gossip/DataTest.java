...
public class DataTest {
    ...
    public static void initializeMembers() throws InterruptedException, UnknownHostException, URISyntaxException {
        ...
        for (Member member : startupMembers) {
            GossipManager gossipService = GossipManagerBuilder.newBuilder().cluster(cluster).uri(member.getUri()).id(
                    member.getId()).gossipMembers(startupMembers).gossipSettings(settings).build();
    ...
    void crdtSetTest(String key, Function<Set<String>, CrdtAddRemoveSet<String, Set<String>, ?>> construct) {
        ...
        //drop element
        @SuppressWarnings("unchecked") CrdtAddRemoveSet<String, ?, ?> set = (CrdtAddRemoveSet<String, ?, ?>) clients.get(
                0).findCrdt(key);
    ...
    public void GrowOnlyCounterTest() {
        Consumer<Long> assertCountUpdated = count -> {
            for (GossipManager client : clients) {
                TUnit.assertThat(() -> client.findCrdt(gCounterKey)).afterWaitingAtMost(10, TimeUnit.SECONDS).isEqualTo(
        ...
        GrowOnlyCounter gc2 = new GrowOnlyCounter(gc, new GrowOnlyCounter.Builder(clients.get(1)).increment(4L));
    ...
    private void assertMergedCrdt(String key, Object expected) {
        for (GossipManager client : clients) {
            TUnit.assertThat(() -> client.findCrdt(key).value()).afterWaitingAtMost(10, TimeUnit.SECONDS).isEqualTo(
                    expected);
        }
    }
