...
public class DataReplicationTest {
    ...
    public void dataReplicateWhiteListTest() throws URISyntaxException {
        ...
        Assert.assertEquals(true, message.getReplicable().shouldReplicate(me, memberList.get(0), message));
        Assert.assertEquals(true, message.getReplicable().shouldReplicate(me, memberList.get(1), message));
        Assert.assertEquals(false, message.getReplicable().shouldReplicate(me, memberList.get(2), message));
    }
    ...
    public void dataReplicateBlackListTest() throws URISyntaxException {
        ...
        Assert.assertEquals(false, message.getReplicable().shouldReplicate(me, memberList.get(0), message));
        Assert.assertEquals(false, message.getReplicable().shouldReplicate(me, memberList.get(1), message));
        Assert.assertEquals(true, message.getReplicable().shouldReplicate(me, memberList.get(2), message));
    }
    ...
    public void dataReplicateDataCenterTest() throws URISyntaxException {
        ...
        memberListDc1.add(getLocalMemberDc(new URI("udp://10.0.0.1:8000"), "1", "DataCenter1", "Rack1"));
        memberListDc1.add(getLocalMemberDc(new URI("udp://10.0.0.2:8000"), "2", "DataCenter1", "Rack2"));
        memberListDc2.add(getLocalMemberDc(new URI("udp://10.0.1.1:8000"), "11", "DataCenter2", "Rack1"));
        memberListDc2.add(getLocalMemberDc(new URI("udp://10.0.1.2:8000"), "12", "DataCenter2", "Rack2"));
        ...
        Assert.assertEquals(
                true, message.getReplicable().shouldReplicate(memberListDc1.get(0), memberListDc1.get(1), message));
        Assert.assertEquals(
                true, message.getReplicable().shouldReplicate(memberListDc2.get(0), memberListDc2.get(1), message));

        // data should not replicate to data center 2
        Assert.assertEquals(
                false, message.getReplicable().shouldReplicate(memberListDc1.get(0), memberListDc2.get(0), message));
        Assert.assertEquals(
                false, message.getReplicable().shouldReplicate(memberListDc1.get(1), memberListDc2.get(1), message));
    }
    ...
    public void dataReplicateDataCenterUnknownDataCenterTest() throws URISyntaxException {
        ...
        LocalMember unknownDc = new LocalMember("cluster1", new URI("udp://10.0.1.2:8000"), "12", 0, properties, 1, 0,
                                                ""
        );

        SharedDataMessage message = getSharedNodeData(
                "datacenter1", "I am in data center 1 rack 1", new DataCenterReplicable<>());

        // data should not replicate from dc1 to unknown node
        Assert.assertEquals(false, message.getReplicable().shouldReplicate(memberListDc1.get(0), unknownDc, message));
        // data can replicate from unknown node to dc
        Assert.assertEquals(true, message.getReplicable().shouldReplicate(unknownDc, memberListDc1.get(0), message));
