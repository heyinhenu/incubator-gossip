...
public class UserDataPersistenceTest {
    ...
    private GossipManager sameService() throws URISyntaxException {
        ...
        return GossipManagerBuilder.newBuilder().cluster("a").uri(
                new URI("udp://" + "127.0.0.1" + ":" + (29000 + 1))).id(nodeId).gossipSettings(settings).build();
    }
    ...
    public void givenThatRingIsPersisted() throws UnknownHostException, InterruptedException, URISyntaxException {
        ...
        { //Create a gossip service and force it to persist its user data
            ...
            {
                ConcurrentHashMap<String, SharedDataMessage> l = gossipService.getUserDataState().readSharedDataFromDisk();
        ...
        { //recreate the service and see that the data is read back in
            ...
            gossipService.init();
            Assert.assertEquals(
                    "red", ((AToothpick) gossipService.findPerNodeGossipData(nodeId, "a").getPayload()).getColor());
