    ...
    public static MessageHandler concurrentHandler(MessageHandler... handlers) {
        ...
        return new MessageHandler() {
            ...
            public boolean invoke(
                    GossipCore gossipCore, GossipManager gossipManager, Base base
            ) {
                // return true if at least one of the component handlers return true.
                return Arrays.asList(handlers).stream().filter(
                        (mi) -> mi.invoke(gossipCore, gossipManager, base)).count() > 0;
            }
        };
    }
}
