...
public class GossipCore implements GossipCoreConstants {
            ...
            if (message.getPayload() instanceof Crdt) {
                ...
                if (replaced) {
                    if (!merged.getPayload().equals(previous.getPayload())) {
                        eventManager.notifySharedData(message.getKey(), merged.getPayload(), previous.getPayload());
                    }
                    return;
                }
            } else {
    ...
    public void mergeLists(RemoteMember senderMember, List<Member> remoteList) {
        ...
        for (Member remoteMember : remoteList) {
            ...
            LocalMember aNewMember = new LocalMember(remoteMember.getClusterName(), remoteMember.getUri(),
                                                     remoteMember.getId(), remoteMember.getHeartbeat(),
    ...
    private void debugState(
            RemoteMember senderMember, List<Member> remoteList
    ) {
        LOGGER.warn(
    ...
    public Crdt merge(SharedDataMessage message) {
        for (; ; ) {
            ...
            @SuppressWarnings("unchecked") Crdt merged = ((Crdt) previous.getPayload()).merge(
                    (Crdt) message.getPayload());
