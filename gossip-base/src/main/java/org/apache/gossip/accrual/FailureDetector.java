    ...
    public synchronized Double computePhiMeasure(long now) {
        ...
        try {
            ...
            if (distribution.equals("normal")) {
                ...
                probability = new NormalDistributionImpl(
                        descriptiveStatistics.getMean(), standardDeviation).cumulativeProbability(delta);
            } else {
                probability = new ExponentialDistributionImpl(descriptiveStatistics.getMean()).cumulativeProbability(
                        delta);
            }
