...
public class StartupSettings {
     ...
     */
    public static StartupSettings fromJSONFile(
            File jsonFile
        ...
        boolean bulkTransfer = jsonObject.has("bulk_transfer") ? jsonObject.get(
                "bulk_transfer").booleanValue() : DEFAULT_BULK_TRANSFER;
        int bulkTransferSize = jsonObject.has("bulk_transfer_size") ? jsonObject.get(
                "bulk_transfer_size").intValue() : DEFAULT_BULK_TRANSFER_SIZE;
        if (cluster == null) {
            throw new IllegalArgumentException("cluster was null. It is required");
        }
        String transportClass = jsonObject.has("transport_manager_class") ? jsonObject.get(
                "transport_manager_class").textValue() : null;
        String protocolClass = jsonObject.has("protocol_manager_class") ? jsonObject.get(
                "protocol_manager_class").textValue() : null;
        URI uri2 = new URI(uri);
        GossipSettings gossipSettings = new GossipSettings(gossipInterval, cleanupInterval, windowSize, minSamples,
                                                           convictThreshold, distribution, bulkTransfer
        );
        ...
        while (it.hasNext()) {
            ...
            RemoteMember member = new RemoteMember(child.get("cluster").asText(), uri3, "", 0,
