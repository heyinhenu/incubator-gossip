...
public class StandAloneDatacenterAndRack extends StandAloneExampleBase {
    ...
    void initGossipManager(String[] args) {
        ...
        GossipManager manager = GossipManagerBuilder.newBuilder().cluster("mycluster").uri(URI.create(args[0])).id(
                args[1]).gossipSettings(s).gossipMembers(
                Arrays.asList(new RemoteMember("mycluster", URI.create(args[2]), args[3]))).properties(props).build();
        manager.init();
